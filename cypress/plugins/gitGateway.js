...
async function setupGitGateway(options) {
  ...
  await enableGitGateway(netlifyApiToken, site_id, provider, token, `${result.owner}/${result.repo}`);
  ...
  await git.raw(["config", "-f", ".lfsconfig", "lfs.url", `https://${site_id}.netlify.com/.netlify/large-media`]);
