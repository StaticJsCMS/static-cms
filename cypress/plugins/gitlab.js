...
function getEnvs() {
  const { GITLAB_REPO_OWNER: owner, GITLAB_REPO_NAME: repo, GITLAB_REPO_TOKEN: token } = process.env;
  if (!owner || !repo || !token) {
    throw new Error("Please set GITLAB_REPO_OWNER, GITLAB_REPO_NAME, GITLAB_REPO_TOKEN environment variables");
  }
...
async function resetOriginRepo({ owner, repo, tempDir }) {
  ...
  await Promise.all(ids.map((id) => client.MergeRequests.edit(projectId, id, { state_event: "close" })));
...
const sanitizeString = (str, { owner, repo, token, ownerName }) => {
  let replaced = str
    ...
    .replace(new RegExp("https://secure.gravatar.+?/u/.+?v=\\d", "g"), `${FAKE_OWNER_USER.avatar_url}`);
...
const transformRecordedData = (expectation, toSanitize) => {
  const requestBodySanitizer = (httpRequest) => {
    ...
    if (httpRequest.body && httpRequest.body.type === "JSON" && httpRequest.body.json) {
      const bodyObject =
        typeof httpRequest.body.json === "string" ? JSON.parse(httpRequest.body.json) : httpRequest.body.json;
  ...
  const responseBodySanitizer = (httpRequest, httpResponse) => {
    ...
    if (responseBody && httpRequest.path === "/api/v4/user" && httpRequest.headers.host.includes("gitlab.com")) {
