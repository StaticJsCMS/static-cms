...
const LocalDateTime = /*#__PURE__*/ fpc(
  class LocalDateTime extends Datetime {
    ...
    constructor(literal) {
      ...
      super();
      this[LocalDateTime_value] = Value((this[LocalDateTime_ISOString] = literal.replace(T, 'T')));
      return this;
    }
  ...
  if (literal === 'true') {
    ...
    if (literal.includes('-')) {
      if (IS_OFFSET$(literal)) {
        ...
        moreDatetime ||
          throws(
            SyntaxError$1(`Local Date-Time is not allowed before TOML v0.5` + where(', which at ')),
          );
        asLocalDateTimes(lastArray)[lastArray.length] = new LocalDateTime(literal);
      }
    } else {
      moreDatetime ||
        throws(SyntaxError$1(`Local Time is not allowed before TOML v0.5` + where(', which at ')));
...
const Root = () => {
  ...
  while (rest()) {
    ...
    if (line) {
      if (line[0] === '[') {
        const { leadingKeys, finalKey, asArrayItem, tag, lineRest } = TABLE_DEFINITION_exec_groups(
          line,
          parseKeys,
        );
