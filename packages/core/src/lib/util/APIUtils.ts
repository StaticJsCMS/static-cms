import { WorkflowStatus, workflowStatusFromString } from '@staticcms/core/constants/publishModes';
...
export function labelToStatus(label: string, labelPrefix: string): WorkflowStatus {
  return (
    workflowStatusFromString(label.slice(getLabelPrefix(labelPrefix).length)) ??
    WorkflowStatus.DRAFT
  );
}

export function statusToLabel(status: WorkflowStatus, labelPrefix: string) {
