...
| filter                | string                                                                                                                             |           | _Optional_. Filter for `folder` collections; details in [Collection Types](/docs/collection-types#filtered-folder-collections)                |
| create                | string                                                                                                                             |           | **For `folder` collections only**<br /><ul><li>`true` - Allows users to create new items in the collection; defaults to `false`                                 |
| hide                  | string                                                                                                                             |           | `true` hides a collection in the CMS UI; defaults to `false`. Useful when using the relation widget to hide referenced collections.           |
| delete                | string                                                                                                                             |           | `false` prevents users from deleting items in a collection; defaults to `true`                                                                |
